...
d3.json(url, function (data) {
  ...
  var colorToValue = function (color) {
      ...
      tooltip
        .html(
          months[d.month - 1] +
            ...
            d.year +
            '<br>Temp: ' +
            (data.baseTemperature + d.variance).toFixed(3) +
            ' °C' +
            '<br>Variance: ' +
            d.variance +
            '<br>'
  ...
  var legend = svg
    ...
    .attr(
      'transform',
      'translate(' +
        (width - legendWidth * colors.length) +
        ',' +
        (margin.bottom + -580) +
        ')'
    );
  ...
  legend
    ...
    .append('text')
    .text(function (d) {
      return formatNum(rounded(colorToValue(d) + baseTemp));
    })
